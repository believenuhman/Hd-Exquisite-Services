      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-4 py-4 flex flex-col gap-5" style={{ paddingBottom: 100 }}>
        ...
        <div className="rounded-2xl p-4" style={{ background: "linear-gradient(145deg, #1C1828, #121212)", border: "1px solid rgba(228,161,43,0.1)" }}>
          ...
          <div className="flex flex-col gap-3">
            {/* Cash on Delivery */}
            <button onClick={() => setPaymentMethod("cash_on_delivery")} className="flex items-center gap-4 rounded-xl p-4 press-active text-left"
              ...
              <div className="flex items-center justify-center rounded-full flex-shrink-0" style={{ width: 40, height: 40, background: paymentMethod === "cash_on_delivery" ? "rgba(228,161,43,0.15)" : "rgba(255,255,255,0.05)" }}>
              ...
              <div className="flex-shrink-0 rounded-full" style={{ width: 18, height: 18, border: `2px solid ${paymentMethod === "cash_on_delivery" ? "#E4A12B" : "rgba(255,255,255,0.2)"}`, background: paymentMethod === "cash_on_delivery" ? "#E4A12B" : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {paymentMethod === "cash_on_delivery" && <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#09090C" }} />}
              </div>
            </button>

            {/* Pay Online */}
            <button onClick={() => setPaymentMethod("online_card")} className="flex items-center gap-4 rounded-xl p-4 press-active text-left"
              style={{ background: paymentMethod === "online_card" ? "rgba(201,30,140,0.08)" : "rgba(255,255,255,0.03)", border: `1.5px solid ${paymentMethod === "online_card" ? "rgba(201,30,140,0.5)" : "rgba(255,255,255,0.08)"}`, transition: "all 0.2s" }}>
              <div className="flex items-center justify-center rounded-full flex-shrink-0" style={{ width: 40, height: 40, background: paymentMethod === "online_card" ? "rgba(201,30,140,0.15)" : "rgba(255,255,255,0.05)" }}>
                <IoCard size={20} color={paymentMethod === "online_card" ? "#C91E8C" : "rgba(255,255,255,0.4)"} />
              </div>
              <div className="flex-1">
        ...
        <button onClick={handlePlaceOrder} disabled={loading} className="w-full py-4 rounded-2xl font-inter font-bold text-sm tracking-widest press-active"
          style={{
            background: loading
              ? "rgba(228,161,43,0.3)"
              : paymentMethod === "online_card"
              ? "linear-gradient(135deg, #C91E8C, #9B15A0)"
